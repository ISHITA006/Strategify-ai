        ...
        for (var j=0; j<rawReviews.length;j++){
          ...
          const result = await client.generateText({model: MODEL_NAME, temperature:0, prompt: {text: prompt,},});
            ...
            if (style.includes('NA')){
              style_rating = 0
              style_aspect = 'NA'
              summary["style"]["0"]= summary["style"]["0"]+1
            }
            else{
              style_rating = style.split(" & ")
              style_aspect = style_rating[1]
              summary["style"][style_rating[0]]= summary["style"][style_rating[0]]+1
            ...
            if (customerService.includes('NA')){
              ...
              summary["customerService"]["0"]= summary["customerService"]["0"]+1
            }
            else{
              customerService_rating = customerService.split(" & ")
              customerService_aspect = customerService_rating[1]
              summary["customerService"][customerService_rating[0]]= summary["customerService"][customerService_rating[0]]+1
            ...
            summary["quality"][quality] = summary["quality"][quality] +1
            newReview["style"] = {"rating": style_rating, "aspect": style_aspect}
            newReview["fit"] = fit
            summary["fit"][fit] = summary["fit"][fit] +1
            newReview["delivery"] = delivery
            summary["delivery"][delivery] = summary["delivery"][delivery] +1
            newReview["valueForMoney"] = valueForMoney
            summary["valueForMoney"][valueForMoney] = summary["valueForMoney"][valueForMoney] +1
